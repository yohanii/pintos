  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      struct vm_entry *vmentry = (struct vm_entry *)malloc(sizeof (struct vm_entry));
      if (vmentry == NULL)
        return false;

      memset (vmentry, 0, sizeof (struct vm_entry));
      vmentry->type = VM_BIN;
      vmentry->file = file;
      vmentry->offset = ofs;
...
handle_mm_fault(struct vm_entry* vme)
{
  printf("handle_mm_fault start!\n");
  struct page *pg = alloc_page(PAL_USER);
	pg->vme = vme;
  printf("aaaaa\n");
  if(pg == NULL){
    return false;
