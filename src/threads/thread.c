...
thread_init (void)
{
  ...
  initial_thread->nice = int_to_fixed(0);
...
{
  int cur_load_avg = fixed_to_int_round(fixed_mult_mix(load_avg,100));
  return cur_load_avg;
...
{
  int cur_recent_cpu = fixed_to_int_round(fixed_mult_mix(thread_current()->recent_cpu,100));
...
void recent_cpu(struct thread *thread){
   ...
   //using fixed point calculation
  if(thread != idle_thread){
    int twice_load_avg = fixed_mult_mix(load_avg, 2);
    int new_load_avg = fixed_div(twice_load_avg, fixed_add_mix(twice_load_avg, 1));
    int new_recent_cpu = fixed_add_mix(fixed_mult(thread->recent_cpu, new_load_avg), thread->nice);
...
void calc_load_avg(){
  ...
  if(thread_current() == idle_thread){
    load_avg = fixed_add (fixed_div_mix(fixed_mult_mix(load_avg, 59), 60),
                fixed_div_mix(int_to_fixed(list_size(&ready_list)), 60));
    }
  else{
    load_avg = fixed_add (fixed_div_mix(fixed_mult_mix(load_avg, 59), 60),
                fixed_div_mix(int_to_fixed(list_size(&ready_list) + 1), 60));
    }
...
void calc_priority(struct thread *thread){
    ...
    if(thread != idle_thread){
      int new_priority = fixed_to_int(fixed_sub(int_to_fixed(PRI_MAX), fixed_add_mix(fixed_div_mix(thread->recent_cpu, 4), thread->nice * 2)));
      if(new_priority > PRI_MAX) new_priority = PRI_MAX;
      if(new_priority < PRI_MIN) new_priority = PRI_MIN;
     // printf("%s thread priority: %d\n", thread->name, new_priority);
...
void recent_cpu_incr(){
   //recent cpu value ++;
    if (thread_current () != idle_thread)
        thread_current ()->recent_cpu = fixed_add_mix (thread_current ()->recent_cpu, 1);
}
